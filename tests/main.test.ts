...
describe('TheFehrsLearningManager', () => {
  ...
  describe('registerSettings', () => {
    it('should register world settings', () => {
      ...
      expect(game.settings.register).toHaveBeenCalledWith(TheFehrsLearningManager.ID, "rules", expect.any(Object));
      expect(game.settings.register).toHaveBeenCalledWith(TheFehrsLearningManager.ID, "timeUnits", expect.any(Object));
      expect(game.settings.register).toHaveBeenCalledWith(TheFehrsLearningManager.ID, "guidanceTiers", expect.any(Object));
      expect(game.settings.register).toHaveBeenCalledWith(TheFehrsLearningManager.ID, "projectTemplates", expect.any(Object));
    });
  });
  ...
  describe('activateListeners', () => {
    it('should attach click listener to bulk-train buttons', () => {
      ...
      // Let's trigger it.
      const processTrainingSpy = vi.spyOn(TheFehrsLearningManager as any, 'processTraining').mockResolvedValue(undefined);
    ...
    it('should attach click listener to add-selected-project button', async () => {
      ...
      vi.mocked(game.settings.get).mockReturnValue([{ id: 'tpl1', name: 'Template 1', target: 100 }]);
      ...
      expect(actor.setFlag).toHaveBeenCalledWith(TheFehrsLearningManager.ID, 'projects', expect.arrayContaining([
